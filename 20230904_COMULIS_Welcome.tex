...
\usepackage{tikz}
\usepackage{xspace}
\usepackage[missing=main]{gitinfo2} % GitHub Actions don't pull in the commit
hash, so we just show `main'
\usepackage{tcolorbox}

% Define us a custom footer *with* progress bar, based on
https://tex.stackexchange.com/a/59749/828
\makeatletter
...
,halign title=center,title=Voucher code]
				%% Get the currently valid guest voucher here:
https://serviceportal.unibe.ch/sp?id=kb_article_view&sysparm_article=KB0010178
				%% Copy-paste it into the `voucher.tex` file, since "The voucher code must
